...
class WebSignupView(View):
    ...
    def create_account(self, user):
        ...
        if not adminc.connect():
            raise Exception(
                "Failed to create account " "(unable to obtain admin connection)"
            )
    ...
    def _get_new_login(self, adminc, user):
        ...
        while n < 99:
            ...
            e = adminc.getObject("Experimenter", attributes={"omeName": checkname})
            if not e:
                return checkname
    ...
    def _get_or_create_group(self, adminc, user):
        ...
        g = adminc.getObject("ExperimenterGroup", attributes={"name": groupname})
        if g:
            gid = g.id
        else:
            logger.info(
                "Creating new signup group: %s %s",
                groupname,
    ...
    def _email_user(self, client, omeuser):
        ...
        cb = client.submit(req, loops=10, ms=500, failonerror=True, failontimeout=True)
        try:
            rsp = cb.getResponse()
